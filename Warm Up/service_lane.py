...
# the type of largest vehicle which can pass through the service lane (including
# the entry & exit segment)

# Input Format The first line of input contains two integers - N & T, where N is
# the length of the freeway, and T is the number of test cases. The next line
# has N space separated integers which represents the width array.
...
# Output Format For each test case, print T number that represents the largest
...
# (0, 2): Because width[2] = 1, only the bike represented as 1 can pass through
# (it.

# (0, 1): Here the largest allowed vehicle which can pass through the 0th
# (segment is car and for the 1st segment, it's the truck. Hence the largest
# (vehicle allowed in these segments is a car.

# (6, 7): In this example, the vehicle enters at the 6th segment and exits at
# (the 7th segment. Both segments allow even truck to pass through them. Hence
# (truck is the answer.

# (3, 5): width[3] = width[5] = 2. While 4th segment allow the truck, 3rd and
# (5th allow upto car. So 2 will be the answer here.

# (0, 7): Bike is the only vehicle which can pass through the 2nd segment, which
# (limits the strength of whole lane to 1.

_, test_cases = raw_input().split()
lanes = map(int, raw_input().split())

for _ in range(int(test_cases)):
    entry, exit = map(int, raw_input().split())
    print min(lanes[entry:exit + 1])
